...
class ProductosCardViewModel{
    var result = Result()
    var context = DBManager()

    
    static func GetbAllProducto(IdDepartamento : Int) -> Result{
        var context = DBManager()
        var result = Result()
        let query = "select IdProducto, Nombre, PrecioUnitario, Descripcion, Imagen From Productos where IdDepartamento = \(IdDepartamento)"
